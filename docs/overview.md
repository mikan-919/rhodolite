...
| ファイル | 役割 | 行 |
|---|---|---|
| `src/lex.rs` | 字句解析 + 行継続 | 404 |
| `src/ast.rs` | 構文木の型定義。ここを読めば言語の形が分かる | 177 |
| `src/parse.rs` | 再帰下降パーサ | 953 |
| `src/requirement.rs` | **要求推論(中核)** | 710 |
| `src/eval.rs` | **評価。`Env` は切れて `Ambient` は切れない** | 1225 |
...
1. **要求推論が `impl` の本体を見ていない。** ハンドラのメソッドがスロットを
   使うと要求が漏れる。正典のハンドラは使っていないので今は無害
2. **型検査が無い。** `struct` の宣言フィールドは照合していない。
   ADR-0003 の whole-program 単相化はここから
3. **enum が無い。** `Gold` はフィールド0個の struct を名札として使っている
