...
| `src/requirement.rs` | **要求推論(中核)** | 855 |
| `src/eval.rs` | **評価。`Env` は切れて `Ambient` は切れない** | 1415 |
...
2. **要求推論が `impl` の本体を見ていない。** ハンドラのメソッドがスロットを
   使うと要求が漏れる。正典のハンドラは使っていないので今は無害
3. **モジュールが無い。** スロット名が増えたときの宣言側の衝突を修飾できない
4. **型検査が無い。** `struct` の宣言フィールドは照合していない。
   ADR-0003 の whole-program 単相化はここから
5. **enum が無い。** `Gold` はフィールド0個の struct を名札として使っている
6. エラーに span が付いていない(`miette` を入れるならここ)
