...
フィールドの読み、演算子の結果、戻り値型を宣言したトップレベル関数の直接
呼び出し、およびそれらを直接束縛・参照する式。

`nil` は期待される `T?` の文脈だけで適合し、`T? ?? T` は `T` になる。`??` の右辺が
直接 `return` なら、その枝は値を産まずに終了できる。裸の `let x = nil` は nominal 型を
決めず、後の代入から遡って推論もしない。

optional field access・配列・メソッドと関連関数の呼び出しはまだ「分からない」に落ちる
ので、そこには診断が出ない。これは**この段の実装がまだ届いていない**だけで、言語の
側にワイルドカード型があるわけではない。後続の一歩が一つずつこの集合を狭める。
型の同一性は名前と後置 `?` の一致だけで、部分型も暗黙の optional 展開も無い。

残っている穴埋めの優先順は次の通り。

1. **optional field access がまだ無い。** `T?`・`nil`・`??` の core は動いた。
   field access の表記と、連鎖した結果の optional 伝播規則を決めて実装する
2. **配列とメソッドが型を持たない。** 要素型、ループ束縛、メソッド候補の
   絞り込み、trait 宣言と impl の突き合わせはこの次。ADR-0003 の
   whole-program 単相化はその続き
3. **enum は最小のまま。** データ付き variant、`match`、網羅性検査、
   限定参照(`Rank::Gold`)は無い
4. エラーに span が付いていない(`miette` を入れるならここ)
