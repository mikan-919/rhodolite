...
- [ ] 1.1 Test: a generic instantiation's callback requirement reaches the
...
- [ ] 1.2 Test: the same generic declaration's two instantiations (same type
...
- [ ] 1.3 Test: the same generic declaration's two instantiations with
...
- [ ] 1.4 Test: a callback forwarded through a chain of two nested generic
...
- [ ] 1.5 Test: a recursive generic helper that forwards its own callback
...
- [ ] 1.6 Test: a missing-provider diagnostic reaching a generic call site
...
- [ ] 2.2 Test: two calls to the same generic instantiation (same type
      arguments, same callback, same providers in scope) share one
      `Instance` via `Plan::intern` — no duplicate instance is created.
- [ ] 2.3 Test: a missing provider reaching a generic instantiation's
...
- [ ] 2.4 Run the full existing `ambient_abi.rs` test suite; confirm no
      existing (non-generic) test's expected output changes.

## 3. Fix the `Analysis` summary-map name collision

- [ ] 3.1 In `requirement::analyze_hir`'s summary-construction loop
...
- [ ] 3.2 Test: a program with two reachable instantiations of one generic
...
- [ ] 3.3 Test: a program with no generic instantiations (or exactly one
...
- [ ] 5.1 `cargo fmt --check` passes.
- [ ] 5.2 `cargo clippy --all-targets -- -D warnings` passes.
- [ ] 5.3 Full `cargo test` suite passes.
- [ ] 5.4 Commit the verified working state as a snapshot, per repo
      convention.
