...
The checker SHALL compare each inferable argument type with the corresponding declared parameter type using nominal type identity and optionality.
...
- **WHEN** a direct top-level function parameter and its inferable argument have different nominal types or optionality
...
For a function with a declared return type, the checker SHALL compare inferable explicit return values and the inferable final body expression with the declared return type.
...
- **WHEN** a function's final expression has an inferable type different from its declared return type
- **THEN** the checker reports the function, expected return type, and actual type

#### Scenario: Mismatched explicit return
- **WHEN** an explicit `return` carries an inferable type different from the containing function's declared return type
- **THEN** the checker reports the function, expected return type, and actual type
