...
- [ ] 1.1 Add an opaque handle type (name is the implementer's choice, e.g.
      ...
      Decision 2).
- [ ] 1.2 Add a handle table (name/type is the implementer's choice, e.g. a
...
- [ ] 1.3 Add the table as a field on `Interp<'p>` behind interior
...
- [ ] 2.1 Add a variant to `eval::Value` that carries the opaque handle type
      from 1.1.
- [ ] 2.2 In `CheckedInterp::public_value`, replace the current
...
- [ ] 2.3 Update `Interp::show` to render the new `Value` variant (any
      readable form is fine; this is CLI/debug display only, not a wire
      format).

## 3. Invoke operation

- [ ] 3.1 Add an invoke method on `Interp` (name is the implementer's
...
- [ ] 3.2 Look up the handle via the table's `take` operation from 1.2
...
- [ ] 3.3 If `take` returns the target `CallableId`, convert the scalar/unit
...
- [ ] 3.4 Convert the target's result back to `eval::Value` via
      `public_value` (reusing 2.2's handle-minting behavior automatically if
      the result is itself callable-typed).

## 4. Tests

- [ ] 4.1 In `eval.rs`'s test module, add a helper that type-checks and
      ...
      relaxation applies.
- [ ] 4.2 Unit test: running a public export that returns a callable value
      naming a zero-ambient-requirement function produces the new `Value`
      handle variant (not a runtime failure).
- [ ] 4.3 Unit test: invoking that handle with matching scalar arguments
      runs the target function and yields the expected result.
- [ ] 4.4 Unit test: invoking the same handle a second time is rejected
...
- [ ] 4.5 Unit test: invoking a handle-shaped value that was never minted by
...
- [ ] 5.3 `cargo test`
- [ ] 5.4 Confirm no files under `src/wasm*.rs` changed (this change is
      scoped to the interpreter's internal handle mechanism only; CAB-030
      owns the Wasm invoke export per ROADMAP.md).
- [ ] 5.5 Commit the verified, working state as a snapshot before moving on.
