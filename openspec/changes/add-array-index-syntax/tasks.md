...
- [ ] 1.1 Add `ExprKind::Index(Box<Expr>, Box<Expr>)` to `src/ast.rs`.
- [ ] 1.2 Add a `Tok::LBracket` case to `postfix()`'s loop in `src/parse.rs`,
      parsing `[` `self.expr()` `]` (via `self.expect(&Tok::RBracket, "`]`")`)
      onto the current expression, at the same precedence tier as
      `.field`/`(args)`/`::path`.
- [ ] 1.3 Update `docs/grammar.md` with the `postfix '[' expr ']'`
      production.
- [ ] 1.4 Add focused parser tests in `src/parse.rs`: simple subscript
...
- [ ] 4.4 Add a focused `module.rs` test confirming a name inside either side
...
- [ ] 4.5 Add a focused `typecheck.rs` test confirming a program using
...
- [ ] 5.2 Confirm this task's scope is grammar-only: no interpreter, Wasm, or
