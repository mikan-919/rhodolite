...
- [x] 1.1 Confirm `trait Map<T> { fn map<U>(self, f: fn(T -> U) -> [U]) }`
      ...
      method" scenario.
- [x] 1.2 Add a focused test that `impl<T> Map<T> for [T] { fn map<U>(self,
      f: fn(T -> U) -> [U]) { ... } }` still reports "not a struct" before
      any widening lands (proves the starting point, not a regression).
- [x] 1.3 Widen `check_generic_impls` (`src/typecheck.rs`) to accept an
...
- [x] 1.4 Confirm a concrete-element array target (`impl<T> Foo<T> for
      [int]`) and a callable-type target still report "not a struct".
- [x] 1.5 Confirm two `impl<T> Map<T> for [T]` declarations (or one
...
- [x] 2.1 Add the array-receiver arm to `resolve()`'s method-call match
...
- [x] 2.2 Focused tests: `xs.map(f)` infers `U` from `f`'s return type;
...
- [x] 3.1 Extend `instance_owner` (`src/typecheck.rs`) to compute `self`'s
...
- [x] 3.2 Run the existing generic-trait-resolution struct-impl test suite
      unmodified; confirm zero regression (verifies the struct-target
      substitution is a byte-for-byte no-op).
- [x] 3.3 Focused test: two `map` instantiations over different element
...
- [x] 4.1 Write `impl<T> Map<T> for [T]`'s `map` body using `let mut
      result: [U] = []`, `for x in move self`, `result.push(f(move x))`,
      `move result` (design.md Decision 4).
- [x] 4.2 Confirm ownership checking accepts the body with zero new code in
...
- [x] 5.3 Run the full existing requirement/ambient test suite; confirm no
...
- [x] 6.1 `map-copy-element` fixture (`src/differential.rs`): `move
      xs.map(f)` over a `Copy` element type.
- [x] 6.2 `map-owned-element` fixture: `move xs.map(f)` over a non-`Copy`
      element type, comparing final ownership state.
- [x] 6.3 `map-empty-array` fixture: `move xs.map(f)` over an empty array.
- [x] 6.4 `map-ambient-callback` fixture: `move xs.map(f)` where `f`
      requires an ambient slot provided around the call.
- [x] 6.5 Register the fixtures in `FIXTURES`; confirm
...
- [x] 7.1 Confirm every scenario in `generic-map`, the modified
      `generic-trait-resolution` requirement, and the modified
      `differential-execution` scenarios has a corresponding test from
      Sections 1-6.
- [x] 7.2 `cargo test` passes in full.
- [x] 7.3 `cargo fmt --check` and `cargo clippy --all-targets -- -D
      warnings` pass.
