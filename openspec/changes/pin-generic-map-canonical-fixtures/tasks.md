...
- [ ] 1.1 Confirm `map-copy-element` (`src/differential.rs`) is the `map`
...
- [ ] 1.2 Confirm `map-owned-element` is the `map` corpus member for the
...
- [ ] 1.3 Confirm `map-ambient-callback` is the `map` corpus member for the
...
- [ ] 1.4 Confirm `mapを通る提供忘れの経路はmapを名指す`
...
- [ ] 1.5 If any of 1.1-1.4 does not already hold, stop and report back
...
- [ ] 2.1 In `src/differential.rs`, add a fourth tuple to `SNAPSHOTS` for
...
- [ ] 2.3 Re-run the same test and confirm it now passes.

## 3. Update the spec text to match

- [ ] 3.1 Sync the `differential-execution` delta from
...
- [ ] 4.1 `cargo test` passes in full (including
      `maintained_corpus_has_matching_observable_outcomes`,
...
- [ ] 4.2 `cargo fmt --check` passes.
- [ ] 4.3 `cargo clippy --all-targets -- -D warnings` passes.
- [ ] 4.4 Confirm two consecutive `prepare()` runs of `map-ambient-callback`
      (already exercised generically by
      `every_fixture_is_byte_deterministic_and_independently_executable`,
      re-verified here for the specific fixture this change pins) produce
      byte-identical Wasm.
- [ ] 4.5 Commit the stable, passing state as a single snapshot.
