...
- [x] 1.1 Add `TypeParam { name, span }` and `type_params: Vec<TypeParam>` to
...
- [x] 1.2 Parse `<T, U>` after the `fn` name (shared by free functions, trait
...
- [x] 1.3 Reject a type parameter list on `struct`/`enum` with a
      source-positioned diagnostic naming the declaration kind.
- [x] 1.4 Update `docs/grammar.md` with the `fn`/`trait`/`impl` type parameter
      syntax and the widened `impl` grammar.
- [x] 1.5 Add focused parser tests: type parameter parsing and spans on
...
- [x] 2.1 Add `hir::TypeParamId` (via the existing `ids!` macro).
- [x] 2.2 In `typecheck.rs`, build each generic declaration's type parameter
...
- [x] 2.3 Resolve `Named(name)` types in a generic declaration's params,
...
- [x] 2.4 Confirm (with a test) that `module::resolve_type` leaves an
      unresolved bare type parameter name unchanged, so it reaches the scope
      check in 2.3 unmodified.
- [x] 2.5 Add focused typecheck tests: duplicate type parameter names (own
...
- [x] 3.1 Add `hir::GenericType` (`Builtin`, `Named`, `Array`, `Callable`,
...
- [x] 3.2 Do not register generic declarations in `Ids::fns` /
...
- [x] 3.3 Add `GenericType::is_concrete` (and a whole-signature variant)
      returning `false` exactly when a `Param` occurs anywhere in the tree.
- [x] 3.4 Add a regression test enumerating `hir::TypeKind`'s variants and
...
- [x] 3.5 Add focused tests: a well-formed generic `fn`/`trait`/`impl`
