...
- [ ] 1.1 Add `Out.callback_bindings: hir::Bindings`, saved/restored around
      `check_body`'s target switch exactly like `out.body` already is
      (`src/typecheck.rs:2921`).
- [ ] 1.2 Add a `callback_key: &[Option<hir::CallableId>]` parameter to
...
- [ ] 1.3 Update the three existing non-generic `check_body` call sites
...
- [ ] 1.4 Run the full existing test suite and confirm byte-identical HIR
      dumps/diagnostics for every non-generic program.

## 2. Compute a call site's callback key

- [ ] 2.1 In `generic_call` (`src/typecheck.rs:2110`), after the existing
...
- [ ] 2.2 Pass `callback_key` into `instantiate()` alongside the existing
      `type_args`.

## 3. Widen the instantiation cache and recursion stack

- [ ] 3.1 Change `Out.instances` to
...
- [ ] 3.2 Update `instantiate()`'s cache lookup and "reserve slot before
      recursing" logic (`src/typecheck.rs:2266-2327`) to compare the full
      triple `(id, type_args, callback_key)`.
- [ ] 3.3 Keep the polymorphic-recursion scan comparing only `(id,
...
- [ ] 3.4 In `instantiate()`, after allocating the shell and before
...
- [ ] 4.1 Unit test: two calls with the same type arguments and the same
      callback binding (e.g. `apply(double, 1)` and `apply(double, 2)`)
      still share one instantiation.
- [ ] 4.2 Unit test: two calls with the same type arguments and different
...
- [ ] 4.3 Unit test: a generic function that forwards its own callback
...
- [ ] 4.4 Unit test: the existing type-argument polymorphic-recursion
      fixture (`grow`) is still rejected with the same diagnostic wording,
      unaffected by the widened key.
- [ ] 4.5 Unit test: the existing generic-impl-method equivalents of 4.1-4.4
      (mirroring MAP-025's `Count`/`Grow` trait fixtures) for
      `generic-trait-resolution`.
- [ ] 4.6 Regression: existing non-callback generic tests (`identity`,
...
- [ ] 4.7 CLI diagnostic test (`tests/cli.rs`): the existing
      `型引数の変わる再帰は実行前に失敗する` /
      `型引数の変わるgeneric_implの再帰は実行前に失敗する` tests still pass
      unchanged.
- [ ] 4.8 CLI/snapshot test: a program exercising 4.2's shape end-to-end
...
- [ ] 5.1 After archiving, update `openspec/specs/generic-function-instantiation/spec.md`'s
...
- [ ] 5.2 Update `openspec/specs/generic-trait-resolution/spec.md`'s
...
- [ ] 6.1 `cargo fmt --check` passes.
- [ ] 6.2 `cargo clippy --all-targets -- -D warnings` passes.
- [ ] 6.3 Full `cargo test` suite passes.
- [ ] 6.4 Commit the verified working state as a snapshot, per repo
      convention.
