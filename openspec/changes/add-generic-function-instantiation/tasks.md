...
- [ ] 1.1 Add `type_params: &'a TypeParamScope` to `Cx` in `src/typecheck.rs`;
...
- [ ] 1.2 Decouple `check_body`'s parameter derivation from `Option<&Sig>`:
...
- [ ] 1.3 Add a focused test asserting a representative non-generic program's
...
- [ ] 1.4 Add a regression test asserting the lexer rejects `#` as an
...
- [ ] 2.1 Add `Ids.generic_fns: BTreeMap<String, hir::GenericFnId>`, populated
      in `collect()` next to the existing `record_generic` call for
      `Item::Fn`.
- [ ] 2.2 Add a conversion from `hir::GenericType` to `KnownType` that maps
      ...
      `Callable` recurse).
- [ ] 2.3 In `check_and_lower`'s second pass, add a sibling arm for
...
- [ ] 2.4 Add focused `typecheck.rs` tests: `identity<T>(x: T -> T) { x }` and
...
- [ ] 3.1 Add `hir::substitute(generic_ty: &GenericType, subst: &BTreeMap<TypeParamId, Type>) -> Type`
      in `src/hir.rs`, mirroring `GenericType::is_concrete`'s shape.
- [ ] 3.2 Add a unification function comparing a `GenericType` (the callee's
...
- [ ] 3.3 In `resolve()`, add a branch for `ExprKind::Ident(name)` matching
...
- [ ] 3.4 Diagnose an unresolved type parameter (no argument bound it) and a
      ...
      both inferred types.
- [ ] 3.5 Once solved, substitute each parameter type via 3.1 and pass each
...
- [ ] 3.6 Add focused `typecheck.rs` tests: `identity(5)` and `identity("hi")`
...
- [ ] 4.1 Add a `Vec<(GenericFnId, Vec<hir::Type>, CallableId)>` cache and a
      `Vec<(GenericFnId, Vec<hir::Type>)>` "currently instantiating" stack,
      threaded through `check_and_lower`'s existing per-run state.
- [ ] 4.2 On a solved call (3.3-3.5) with no cache hit: check the stack first
      ...
      `CallableId` directly.
- [ ] 4.3 Wire the resolved/cached `CallableId` into `CallTarget::Direct`, so
      the calling body's `hir::Call::Direct` is indistinguishable from an
      ordinary direct call.
- [ ] 4.4 Add focused `typecheck.rs` tests: two calls to `identity` with the
...
- [ ] 5.1 Add `tests/cli.rs` success cases: a program calling `identity` at
...
- [ ] 5.2 Add `tests/cli.rs` failure cases: unresolved type argument,
      conflicting type argument, and polymorphic recursion, each asserting
      the rendered diagnostic text and span.
- [ ] 5.3 Add an ownership-focused test exercising `apply`'s consuming
...
- [ ] 5.4 Add a differential/Wasm test building the `identity`/`apply`
      ...
      of the same source.
- [ ] 5.5 Run the full existing test suite, `cargo fmt --check`, and
      ...
      call.
- [ ] 5.6 Sync `generic-type-parameters`'s delta spec in this change into
