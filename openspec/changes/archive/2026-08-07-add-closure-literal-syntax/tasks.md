...
- [x] 1.1 Add `ExprKind::Closure { params: Vec<Param>, ret: Option<Type>,
      body: Vec<Expr> }` to `src/ast.rs`.
- [x] 1.2 Extract `sig()`'s parameter-list-then-optional-return parsing
      (`src/parse.rs`) into a shared `fn fn_params_and_ret(&mut self) ->
      PResult<(Vec<Param>, Option<Type>)>` helper; update `sig()` to call it
      with no behavior change.
- [x] 1.3 Add a `Tok::Fn` case to `primary()` in `src/parse.rs`: `self.bump()`,
      `self.expect(&Tok::LParen, "`(`")?`, `fn_params_and_ret()`,
      `self.expect(&Tok::RParen, "`)`")?`, `self.block()?`, producing
      `ExprKind::Closure`.
- [x] 1.4 Update `docs/grammar.md`'s "名前付き関数の値" section: add the
...
- [x] 1.5 Add focused parser tests in `src/parse.rs`: zero-parameter closure
...
- [x] 2.1 Add a focused parser test proving a closure literal's `params`/
...
- [x] 3.1 Add focused parser tests for malformed closure literals: missing
      ...
      omission.
- [x] 3.2 Add a `tests/cli.rs` case asserting the unclosed-body (or missing
      parameter type) diagnostic's rendered text and source-position
      excerpt.

## 4. Downstream compile-plumbing (mechanical, minimal semantics)

- [x] 4.1 Add an `ExprKind::Closure` arm to `resolve_expr` in `src/module.rs`:
...
- [x] 4.2 Add the matching `ExprKind::Closure` arm to `collect_expr_paths`
      in `src/module.rs`, same shape.
- [x] 4.3 Add an `ExprKind::Closure` arm to `walk_kind`'s value-position
...
- [x] 4.4 Add a focused `module.rs` test confirming a closure body can
...
- [x] 4.5 Add a focused `typecheck.rs` test confirming a program using a
...
- [x] 5.2 Confirm this task's scope is grammar/type-representation-only: no
