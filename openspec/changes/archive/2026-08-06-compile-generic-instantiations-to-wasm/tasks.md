...
- [x] 1.1 Add a new inline fixture constant to `src/differential.rs`
...
- [x] 1.2 Run `cargo test --bin rhodolite differential::` and confirm the
...
- [x] 1.3 If either test fails, diagnose and fix the gap in
...
- [x] 2.1 Add a generic-instantiation instance-signature/ambient-field
...
- [x] 2.2 Add a test asserting that two distinct callback bindings of the
...
- [x] 2.3 Add a test asserting an unreached generic declaration compiles to
...
- [x] 3.1 Add a bounded-loop, capped-page test to `src/wasm.rs::mod tests`
      ...
      leaking it.
- [x] 3.2 If the test traps unexpectedly (OOM within the page cap),
      diagnose and fix the leak in the affected Wasm backend file(s) before
      proceeding.

## 4. Specs

- [x] 4.1 Confirm `openspec/changes/compile-generic-instantiations-to-wasm/
...
- [x] 5.1 Run the full test suite (`cargo test --bin rhodolite`) and confirm
      all existing and new tests pass.
- [x] 5.2 Run `cargo fmt --check` and Clippy with warnings as errors; fix
      any findings.
- [x] 5.3 Commit the stable, passing state as a single snapshot.
- [x] 5.4 Mark MAP-070 `done` in `ROADMAP.md`.
