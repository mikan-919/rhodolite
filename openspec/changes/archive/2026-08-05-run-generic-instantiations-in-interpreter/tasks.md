...
- [x] 1.1 Test: `identity<T>(x: T -> T) { x }` called with two different
...
- [x] 1.2 Test: `apply<T, U>(f: fn(T -> U), x: T -> U) { f(x) }` called
...
- [x] 1.3 Test: a generic trait method instantiation (`trait Box<T> { fn
      ...
      method.
- [x] 1.4 Test: `apply<T, U>(f: fn(T -> U), x: T -> U) { f(move x) }` called
...
- [x] 1.5 Test: the same generic declaration called twice in one program
...
- [x] 1.6 Test: a generic function's callback reads an ambient value
...
- [x] 1.7 Test: the same generic declaration as 1.6, in the same program,
...
- [x] 1.8 Run the full existing `src/eval.rs` test suite; confirm no
      existing (non-generic) test's expected output changes.

## 2. Fix any discovered gap

- [x] 2.1 If any test in section 1 fails against the current interpreter,
...
- [x] 3.1 Confirm `specs/generic-instantiation-execution/spec.md`'s
      scenarios each map to one of the tests added in section 1.
- [x] 3.2 Update `ROADMAP.md`'s MAP-060 row status to `done` once complete,
      per the repo's existing roadmap-tracking convention.

## 4. Quality gates

- [x] 4.1 `cargo fmt --check` passes.
- [x] 4.2 `cargo clippy --all-targets -- -D warnings` passes.
- [x] 4.3 Full `cargo test` suite passes.
- [x] 4.4 Commit the verified working state as a snapshot, per repo
      convention.
