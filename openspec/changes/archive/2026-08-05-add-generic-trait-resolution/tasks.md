...
- [x] 1.3 Add focused `hir.rs` tests for `substitute_generic`: a bound
...
- [x] 2.1 In `collect()`'s `Item::Impl` branch, when a generic impl's
...
- [x] 2.2 When it does resolve, compare `trait_ref.args.len()` against the
      resolved trait's `type_params.len()` (task 1.1); on a mismatch, push
      a diagnostic naming the trait and both counts.
- [x] 2.3 Reject a generic impl whose target's outer shape (after
      ...
      target shapes.
- [x] 2.4 Add a `(struct name, trait name) -> Span` tracking table
...
- [x] 2.5 Add focused `typecheck.rs` tests for each of 2.1-2.4: unknown
...
- [x] 3.1 Add a function building, for one trait, a uniform contract table
...
- [x] 3.2 Add trait-level substitution: build `subst_trait` from the trait's
      ...
      ret.
- [x] 3.3 Add method-level positional correspondence: zip the trait method's
...
- [x] 3.4 Compare the fully impl-scoped expected signature against the
      ...
      `GenericType`.
- [x] 3.5 Report a trait-declared method the impl never provides ("missing"),
      and a method name the impl declares twice ("duplicated"), reusing
      `check_impl`'s existing wording.
- [x] 3.6 Wire 3.1-3.5 into `collect()`'s (or a dedicated post-declaration)
...
- [x] 3.7 Add focused `typecheck.rs` tests: a matching generic impl is
...
- [x] 4.1 Replace body-checking's `continue` over a generic impl method
...
- [x] 4.2 Build the method's receiver binding as the ordinary, concrete
...
- [x] 4.3 Add focused `typecheck.rs` tests mirroring MAP-020's generic-body
...
- [x] 5.1 Add `Ids.generic_impls: BTreeMap<(String struct name, String
...
- [x] 5.2 In `resolve()`'s `ExprKind::Field` arm, after today's existing
...
- [x] 5.3 Zero candidates: fall through to the existing "呼び出し先が決まり
      ません" diagnostic unchanged.
- [x] 5.4 More than one candidate: diagnose ambiguity, naming the receiver's
      type, the method name, and each candidate's trait name and
      declaration span, before inferring anything.
- [x] 5.5 Exactly one candidate: `synth` each call argument and `unify()`
...
- [x] 5.6 Add focused `typecheck.rs` tests: a single-candidate call infers
...
- [x] 6.1 Add a `Vec<((hir::StructId, hir::TraitId), hir::TraitImplId)>`
...
- [x] 6.2 On a solved call (5.5) with no existing instantiation-cache entry:
      ...
      `CallableOwner::Free`.
- [x] 6.3 Wire the resolved/cached `CallableId` into `CallTarget::Method
      { callable, recv }`, applying `conform_receiver` unchanged.
- [x] 6.4 Add focused `typecheck.rs` tests: two calls to the same generic
...
- [x] 7.1 Add `tests/cli.rs` diagnostic tests: unknown trait reference,
...
- [x] 7.2 Add a `tests/cli.rs` success case: a program declaring a
...
- [x] 7.3 Add a differential/Wasm test building the program from 7.2 for
...
- [x] 7.4 Run the full existing test suite, `cargo fmt --check`, and
      ...
      unchanged.
- [x] 7.5 Sync `generic-type-parameters`'s delta spec in this change into
