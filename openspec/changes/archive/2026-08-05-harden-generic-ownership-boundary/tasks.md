...
- [x] 1.1 Add a test with a generic free function (e.g. `identity<T>`) called
...
- [x] 1.2 Add the same scenario for a generic impl method (an `impl<T>` on a
...
- [x] 2.1 Add a test where a generic free function instantiated at a
...
- [x] 2.2 Add the same scenario for a generic impl method's consuming `self`
      or non-`Copy` parameter.

## 3. Drop planning

- [x] 3.1 Add a test where a generic free function instantiated at a
...
- [x] 3.2 Add the same scenario for a generic impl method's body.

## 4. Consuming callback moves exactly once

- [x] 4.1 Add the generic impl method counterpart of the existing
...
- [x] 5.1 Add a test compiling a program with at least two distinct
...
- [x] 6.1 If any test added in sections 1-5 fails against current
      `src/ownership.rs`/`src/typecheck.rs`, fix the gap (do not weaken or
      skip the test) before proceeding.
      → ギャップ無し。1〜5 のテストは現行の `src/ownership.rs`/
      `src/typecheck.rs` にそのまま通ったので production code の変更は無い
      (design.md 決定1/決定3 の想定どおり)。

## 7. Verification and snapshot

- [x] 7.1 Run the full test suite (`cargo test`) and confirm all new and
      existing tests pass.
- [x] 7.2 Run `cargo fmt --check` and Clippy with warnings denied
      (`cargo clippy --all-targets -- -D warnings`); fix any findings.
- [x] 7.3 Commit the verified, passing state as a single snapshot per the
